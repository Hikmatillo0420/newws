...
public class service {
    ...
    public static void showDetail() throws SQLException {
        String querry;
        ...
        switch (choice){
            case 1:
                querry = """
                        ...
                        select u_user.id as id,username,age,address,information,linkedin_link,skill_name,level,job_title,company_name,school_name,education.from_date as edfrom,history.to_date as edto,v.title
                                                                                                                      from u_user inner join public.candidate_resume resume on u_user.id = resume.candidate_id
                                                                                                                                  ...
                                                                                                                                  inner join public.vacancy_candidate vc on u_user.id = vc.candidate_id
                                                                                                                                  inner join public.vacancy v on vc.vacancy_id = v.id
                                                                                                                                  ;
                            """;
            ...
            case 5:
                ...
                querry ="insert into vacancy_candidate(vacancy_id,candidate_id,status) values(?,?,?::status_vacancy)";
                String querry2 ="update vacancy set is_active = false where id = ?";
                try (PreparedStatement statement2 = connection.prepareStatement(querry)){
                    System.out.printf("vacancy id: ");
                    int vacancyID = new Scanner(System.in).nextInt();
                    statement2.setInt(1,vacancyID);

                    System.out.printf("candidate id: ");

                    int candidateID = new Scanner(System.in).nextInt();
                    statement2.setInt(2,candidateID);

                    String status = "viewed";
                    statement2.setString(3,status);
                    statement2.executeUpdate();

                    PreparedStatement statement3 = connection.prepareStatement(querry2);
                    statement3.setInt(1,vacancyID);
    ...
    public static void insertDetail() throws SQLException {
        ...
        switch (choice) {
            ...
            case 3:
                ...
                querry ="insert into vacancy(company_id,title,needed_tech,location,work_time,maosh,is_active) values(?,?,?,?,?,?,?)";
                try (PreparedStatement statement2 = connection.prepareStatement(querry)){
                    ...
                    System.out.println("Enter needed tech (enter 'end' to finish):");
                    List<String> needed_tech = new ArrayList<>();
                    while (true) {
                        String tech = scanner.nextLine();
                        if (tech.equals("end")) {
                            break;
                        }
                        needed_tech.add(tech);
                    }
                    ...
                    String work_time = scanner.nextLine();

                    System.out.println("Enter maosh:");
                    float maosh = scanner.nextFloat();

                    System.out.println("Enter is_active (true/false):");
                    boolean is_active = scanner.nextBoolean();

                    statement2.setInt(1, company_id);
                    statement2.setString(2, title);
                    statement2.setArray(3, connection.createArrayOf("text", needed_tech.toArray()));
                    statement2.setString(4, location);
                    statement2.setString(5, work_time);
                    statement2.setFloat(6, maosh);
                    statement2.setBoolean(7, is_active);
